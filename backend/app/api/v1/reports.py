...
async def create_report(user_id: int, payload: ReportCreate, db: AsyncSession = Depends(get_session), current_user=Depends(verify_owner)):
...
async def list_reports(user_id: int, db: AsyncSession = Depends(get_session), current_user=Depends(verify_owner)):
    return await crud.list_reports(db, user_id)


@router.post("/users/{user_id}/reports/generate", response_model=ReportOut)
async def generate_report(user_id: int, payload: Optional[ReportCreate] = None, db: AsyncSession = Depends(get_session), current_user=Depends(verify_owner)):
