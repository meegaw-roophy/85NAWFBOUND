...
async def register(payload: UserCreate, db: AsyncSession = Depends(get_session)):
    ...
    user = await crud.create_user(db, username=payload.username, email=payload.email, password_hash=pw_hash, full_name=payload.full_name, dob=payload.dob, current_location=payload.current_location, country_code=payload.country_code, currency=payload.currency, language=payload.language, primary_goal=payload.primary_goal, is_verified=True)
