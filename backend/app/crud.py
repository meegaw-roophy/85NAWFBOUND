...
async def create_user(db: AsyncSession, username: str, email: str, password_hash: str, full_name: Optional[str] = None, dob: Optional[date] = None, current_location: Optional[str] = None, country_code: Optional[str] = None, currency: Optional[str] = None, language: Optional[str] = None, primary_goal: Optional[str] = None, is_verified: bool = False) -> User:
    user = User(username=username, email=email, password_hash=password_hash, full_name=full_name, dob=dob, current_location=current_location, country_code=country_code, currency=currency or 'USD', language=language, primary_goal=primary_goal, is_verified=is_verified)
    db.add(user)
    await db.commit()
